define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'template'], function ($, undefined, Backend, Table, Form, Template) {
    ...
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'cms/archives/index' + location.search,
                    ...
                    [
                        ...
                        },
                        {
                            field: 'title', title: __('Title'), align: 'left', operate: 'like', customField: 'flag', formatter: function (value, row, index) {
                                // 优先显示中文多语言标题；后备为接口原始title
                                // 兼容后端可能返回的字段名：ac_title / content_title / cn_title
                                var displayTitle = row.cn_title || row.content_title || row.ac_title || value || '-';
                                var flagObj = $.extend({}, this, {searchList: Config.flagList});
                                return '<div class="archives-title"><a href="' + row.url + '" target="_blank"><span style="color:' + (row.style_color ? row.style_color : 'inherit') + ';font-weight:' + (row.style_bold ? 'bold' : 'normal') + '">' + displayTitle + '</span></a></div>' +
                        ...
                        },
                        {
                            ...
                            formatter: function (value, row, index) {
                                ...
                                var html = [];
                                html.push('<span class="label label-info">ID: ' + value + '</span>');
                                if (row.test_type) {
                                    var typeMap = {
                                        'mbti': 'MBTI',
                                        'score': '评分',
                                    ...
                                    html.push('<span class="label label-primary">' + (typeMap[row.test_type] || row.test_type) + '</span>');
                                }
                                if (row.question_count) {
                                    html.push('<span class="label label-success">' + row.question_count + '题</span>');
                                }
                                if (row.test_status) {
                                    var statusClass = row.test_status === '启用' ? 'label-success' : 'label-danger';
                                    html.push('<span class="label ' + statusClass + '">' + row.test_status + '</span>');
                                }
                                return html.join(' ');
                            }
                        },
                        ...
                        },
                        {
                            ...
                            formatter: function (value, row, index) {
                                ...
                                var html = [];
                                var langMap = {'zh': '中文', 'en': 'English'};
                                $.each(langs, function (i, lang) {
                                    var labelClass = lang === 'zh' ? 'label-primary' : 'label-success';
                                    html.push('<span class="label ' + labelClass + '">' + (langMap[lang] || lang) + '</span>');
                                });
                                return html.join(' ');
                            }
                        },
                        ...
                        {field: 'status', title: __('Status'), searchList: {"normal": __('Status normal'), "hidden": __('Status hidden'), "rejected": __('Status rejected'), "pulloff": __('Status pulloff')}, formatter: Table.api.formatter.status},
                        {
                            ...
                            formatter: function (value, row, index) {
                                ...
                                // 添加多语言管理按钮
                                buttons.push('<a class="btn btn-xs btn-info btn-multilang" data-id="' + row.id + '" title="多语言管理"><i class="fa fa-language"></i></a>');
                                
                                // 添加查看详情按钮
                                buttons.push('<a class="btn btn-xs btn-success btn-detail" href="javascript:;" data-id="' + row.id + '" title="查看完整信息"><i class="fa fa-eye"></i></a>');
            ...
            // 多语言管理按钮事件
            $(document).on('click', '.btn-multilang', function () {
                var id = $(this).data('id');
                var url = 'cms/archives/multilang/ids/' + id;
                Fast.api.open(url, '多语言管理', {
                    area: ['90%', '90%']
                });
                return false;
            });
            ...
            $(document).on('click', '.btn-detail', function () {
                var id = $(this).data('id');
                var url = 'cms/archives/detail/ids/' + id;
                Fast.api.open(url, '文章完整信息', {
                    area: ['95%', '95%']
                });
                return false;
            });
        ...
        multilang: function () {
            ...
            $('#multilang-form').on('submit', function(e) {
                ...
                var articleId = $('input[readonly]', this).val();
                
                Fast.api.ajax({
                    url: 'cms/archives/edit/ids/' + articleId,
                    type: 'POST',
                    data: formData
                }, function(data, ret) {
                    Toastr.success('保存成功');
        ...
        api: {
            ...
            bindevent: function () {
                ...
                //提取内容
                $(document).on("click", ".get_con_btn", function (a) {
                    console.log($("#c-get_url").val())
                    Fast.api.ajax({
                        url: "cms/ajax/get_con",
                        data: {get_url: $("#c-get_url").val()}
                    }, function (data, ret) {
                        $('#c-title').val(data.title);
                        $('#c-seotitle').val(data.title);

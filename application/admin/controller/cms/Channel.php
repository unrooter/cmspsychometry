...
//                var_dump($vc['id']);
                \app\admin\model\cms\Channel::refreshItems($vc['id']);
            }
    ...
    public function sync() {
        $api_type = $this->request->post("api_type");
        $api_key = config('site.user_api_key');
        $post_data['api_key'] = $api_key;
        $post_data['api_type'] = $api_type;
        $channel_data = config('site.channel_data');
        if($api_type == 1){
            $post_data['sign'] = getsign($post_data,$api_key);
            $url = "http://api.nbmgkj.com/api/sync/getExams";
            ...
            $data = $res_arr['data']['data'];
            foreach($data as $k => $v){
                $has = \app\admin\model\cms\Archives::where(['product_id' =>$v['product_id']])->find();
                if(empty($v['channel_id'])){
                    continue;
                }
                $add_data['channel_id'] = isset($channel_data[$v['channel_id']])?$channel_data[$v['channel_id']]:$v['channel_id'];
                $add_data['title'] = $v['title'];
                $add_data['sub_title'] = $v['intro'];
                $add_data['question_num'] = $v['question_num'];
                $add_data['product_id'] = $v['product_id'];
                $add_data['model_id'] = 2;
                $add_data['image'] = $v['image'];
                $add_data['seotitle'] = $v['seotitle'];
                $add_data['keywords'] = $v['keywords'];
                $add_data['description'] = mb_substr($v['description'],0,450,'utf-8');
                $add_data['lang'] = $v['lang'];
                $add_data['createtime'] = time();
                $add_data['updatetime'] = time();
                $add_data['publishtime'] = time() - rand(0, 3600 * 24 * 5);
                $add_data['status'] = 'normal';
                $has = Db::table("fa_cms_archives")->where(['product_id' => $v['product_id']])->find();
                if (empty($has)) {
                    $a_id = Db::table("fa_cms_archives")->insertGetId($add_data);
                }else{
                    $a_id = $has['id'];
                    $a_res = Db::table("fa_cms_archives")->where(['id' =>$a_id])->update($add_data);
                }
                $has_addon = Db::table("fa_cms_addonproduct")->where(['id' => $a_id])->find();
                $add_data2 = [];
                $add_data2['content'] = trim($v['content']);
                $add_data2['back_type'] = $v['back_type'];
                $add_data2['show_type'] = $v['show_type'];
                $add_data2['intro'] = $v['intro'];
                if (empty($has_addon)) {
                    $add_data2['id'] = $a_id;
                    $res = Db::table("fa_cms_addonproduct")->insertGetId($add_data2);
                }else{
                    $res = Db::table("fa_cms_addonproduct")->where(['id' => $a_id])->update($add_data2);
                }
            }
        }
        $this->success('同步成功');

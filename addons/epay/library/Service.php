...
class Service
{
    ...
    public static function submitOrder($amount, $orderid = null, $type = null, $title = null, $notifyurl = null, $returnurl = null, $method = null, $openid = '', $custom = [])
    {
        ...
        $type = isset($params['type']) && in_array($params['type'], ['alipay', 'wodpay', 'wechat']) ? $params['type'] : 'wechat';
        ...
        if ($type == 'alipay') {
            ...
            if ($method == 'web') {
                Session::set("wodpayorderdata", $params);
                $url = addon_url('epay/api/wodpay', [], true, true);
                return new RedirectResponse($url);
            }else{
                $appid = $config['uid'];
                ...
                $postData = array(
                    'good_name'=>'购买会员',
                    'pay_type'=>$wod_pay_type,
                    'is_h5'=>'1',
                    'notify_url'=>$params['notifyurl'],
                    'order_id'=>$orderid,
                    'order_uid'=>'uid_'.$auth->id,
                    'price'=>$amount*1,
                    'return_url'=>$params['returnurl'],
                    'uid'=>$appid  //后台基本信息查看
                );
                $postData['sign'] = md5($postData['good_name'].$postData['pay_type'].$postData['notify_url'].$postData['order_id'].$postData['price'].$postData['return_url']. $postData['is_h5']. $postData['uid'].$appsecret);
                $api_url = "http://wod.manguanglife.cn/api.php/pay/getpay";//正式环境以文档为准
            ...
            $result_syt = file_get_contents($api_url, false, $context);
            $result_arr = json_decode($result_syt, true);
            //$result_syt = '{"code":"1","wod_order_id":"965a855028395b1edd0383c0cae75f6b","price":"29","pay_money":"29.00","pay_type":"ali","expire_time":1720590528,"h5url":"https:\/\/qr.alipay.com\/fkx197323rcnwm6dleavkb2","pay_url":"https:\/\/qr.alipay.com\/fkx197323rcnwm6dleavkb2","old_img":"http:\/\/wod.manguanglife.cn\/uploads\/20220114\/82ed69353c188f1f6f0c1262ac3e0c1c.jpg"}';
                $payData['data'] = $result_arr['data'];
                $payData['qr_code'] = $payData['data']['pay_url'];
                return $payData;
            }
        } else {
            //如果是PC支付,判断当前环境,进行跳转

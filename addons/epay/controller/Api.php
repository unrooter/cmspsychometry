...
class Api extends Controller
{
    ...
    public function wodpay()
    {
        if ($this->request->isAjax()) {
            $orderid = $this->request->post("orderid");
            $order_pre = mb_substr($orderid,0,1);
            if($order_pre == 'r'){
                $result = \addons\recharge\model\Order::where(['orderid' => $orderid])->find();
            }else{
                $result = \addons\vip\model\Order::where(['orderid' => $orderid])->find();
            }
            if ($result && $result['status'] == 'paid') {
                $this->success("", "", ['status' => 'TRADE_SUCCESS']);
            } else {
                $this->error("查询失败");
            }
        }
        //发起PC支付(Scan支付)(PC扫码模式)
        $orderData = Session::get("wodpayorderdata");
        if (!$orderData) {
            $this->error("请求参数错误");
        }
    ...
    public function notifywod(){
        $config = Service::getConfig('wodpay');
        $appsecret = $config['secret'];
        $wod_order_id = $_POST['wod_order_id'];
        $order_id = $_POST['order_id'];
        $price = $_POST['price'];
        $pay_money = $_POST['pay_money'];
        $sign =  md5($order_id . $wod_order_id . $price . $pay_money .$appsecret);
        if($sign != $_POST['sign']){
            exit('sign not right');
        }
        \addons\vip\library\Order::settle($order_id, $price);
    }
    public function notifywodr(){
        $config = Service::getConfig('wodpay');
        $appsecret = $config['secret'];
        $wod_order_id = $_POST['wod_order_id'];
        $order_id = $_POST['order_id'];
...
//            exit('sign not right');
//        }
        \addons\recharge\library\Order::settle($order_id, $price);
    }
    public function returnwod()
    {
        $this->success("支付成功!", "index/vip/viplist");
    }
    public function returnwodr()
    {

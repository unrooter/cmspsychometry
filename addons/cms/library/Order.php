...
class Order
{
     ...
     */
    public static function submit($ua_id = 0, $paytype = 'wechat', $method = 'web', $openid = '', $notifyurl = '', $returnurl = '')
    {
        $answer_info = Answer::get($ua_id);
        if (!$answer_info) {
            throw new OrderException('未找到指定查询解析');
        }
        $archives = Archives::get($answer_info['aid']);

        $order = \addons\cms\model\Order::where('ua_id', $ua_id)
    ...
    public static function settle($orderid, $payamount, $memo = '')
    {
        ...
        if ($order['status'] != 'paid') {
            ...
            try {
                ...
                //这个对应的答案支付状态更改
                $ua_info = Answer::get($order['ua_id']);
                if($ua_info['pay_type'] == 0){
                    $ua_res = Answer::where(['id'=>$ua_info['id']])->update(['pay_type'=>7,'paytime'=>time()]);
                }

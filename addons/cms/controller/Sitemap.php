...
class Sitemap extends Base
{
    ...
    {
        $pagesize = $this->request->param('pagesize/d', $this->config['pagesize']);
        $archivesList = \addons\cms\model\Channel::where('status', 'normal')->cache($this->config['cache'])->field('id,name,diyname,createtime,updatetime')->paginate($pagesize, true);
        $list = [];
        foreach ($archivesList as $index => $item) {
            $list[] = [
                'loc'        => $item->fullurl,
                'lastmod'    => date('c', $item->updatetime ?: $item->createtime),
                'changefreq' => 'weekly',
                'priority'   => 0.7
            ];
        }
    ...
    {
        $pagesize = $this->request->param('pagesize/d', $this->config['pagesize']);
        $archivesList = \addons\cms\model\Archives::with(['channel'])->where('status', 'normal')->cache($this->config['cache'])->field('id,channel_id,diyname,createtime,updatetime,publishtime')->paginate($pagesize, true);
        $list = [];
        foreach ($archivesList as $index => $item) {
            $list[] = [
                'loc'        => $item->fullurl,
                'lastmod'    => date('c', $item->updatetime ?: $item->publishtime ?: $item->createtime),
                'changefreq' => 'monthly',
                'priority'   => 0.9
            ];
        }
    ...
    {
        $pagesize = $this->request->param('pagesize/d', $this->config['pagesize']);
        $tagsList = \addons\cms\model\Tag::where('status', 'normal')->cache($this->config['cache'])->field('id,name,createtime,updatetime')->paginate($pagesize, true);
        $list = [];

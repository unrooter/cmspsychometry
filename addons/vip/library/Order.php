...
class Order
{
     ...
     * @throws Exception
     */
    public static function submit($vip_id, $record_id,$aid=0, $amount, $paytype = 'wechat', $method = 'web', $openid = '', $notifyurl = '', $returnurl = '')
    {
        $auth = Auth::instance();
        ...
        if($paytype == 'wodpay'){
            $notifyurl = $request->root(true) . '/addons/epay/api/notifywod/paytype/wodpay';
            if($aid > 0){
                $returnurl = $request->root(true) . '/p/ceshiye.html?aid='.$aid;
            }else{
                $returnurl = $request->root(true) . '/addons/epay/api/returnwod/paytype/wodpay';
            }
        }else{

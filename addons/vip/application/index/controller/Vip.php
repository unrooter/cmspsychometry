...
class Vip extends Frontend
{
    ...
    public function viplist()
    {
        ...
        foreach ($paytypeListArr as $index => $item) {
            if($item == 'wodpay'){
                $paytypeList[] = ['value' => $item, 'image' => '/assets/addons/vip/img/alipay.png', 'default' => $item === $defaultPaytype];
            }else{
    ...
    public function submit()
    {
        ...
        if (!$recordInfo) {
            ...
            \app\admin\model\vip\Record::where(['id'=>$recordInfo['id']])->update(['aid'=>$aid]);
            $recordInfo = Record::where(['id'=>$recordInfo['id']])->find();
        }

        try {
            $response = \addons\vip\library\Order::submit($vipInfo->id, $recordInfo->id, $aid,$recordInfo->amount, $paytype);

...
class Order
{
     ...
     * @throws Exception
     */
    public static function submit($money, $paytype = 'wechat',$aid=0, $method = 'web', $openid = '', $notifyurl = '', $returnurl = '')
    {
        $auth = Auth::instance();
        ...
        if($paytype == 'wodpay'){
            $notifyurl = $request->root(true) . '/addons/epay/api/notifywodr/paytype/wodpay';
            if($aid > 0){
                $returnurl = $request->root(true) . '/p/ceshiye.html?aid='.$aid;
            }else{
                $returnurl = $request->root(true) . '/addons/epay/api/returnwodr/paytype/wodpay';
            }
        }else{
    ...
    public static function settle($orderid, $payamount, $memo = '')
    {
        ...
        if ($order['status'] != 'paid') {
            ...
            if($order['aid'] > 0){
                $answer_info = Answer::where(['id'=>$order['aid']])->find();
                if($answer_info){
                    $archives = Archives::get($answer_info['aid']);
                }
                $request = Request::instance();
                if ($payamount >= $archives->price) {
                    ...
                    $orderone = \addons\cms\model\Order::create($data);
                    User::money(-$archives->price, $answer_info['user_id'], '购买付费文档:' . $archives['title']);
                    \addons\cms\library\Order::settle($orderone->orderid, $archives->price);
                }
            }

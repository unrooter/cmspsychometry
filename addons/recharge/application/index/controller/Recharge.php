...
class Recharge extends Frontend
{
    ...
    public function recharge()
    {
        ...
        foreach (explode(',', $config['paytypelist']) as $index => $item) {
            if($item == 'wodpay'){
                $paytypeList[] = ['value' => $item, 'image' => '/assets/addons/recharge/img/alipay.png', 'default' => $item === $config['defaultpaytype']];
            }else{
    ...
    public function submit()
    {
        ...
        try {
            $response = Order::submit($money, $paytype ? $paytype : 'wechat',$aid);
        } catch (Exception $e) {
